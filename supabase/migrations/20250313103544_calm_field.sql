/*
  ...
  1. Changes
    - Add missing leave type enum values
    - Fix leave request status handling
    - Update leave balance calculation
*/

-- Drop existing leave_type_enum if it exists
DROP TYPE IF EXISTS leave_type_enum CASCADE;

-- Create new leave_type_enum with all required values
...
-- Drop existing leave_status_enum if it exists
DROP TYPE IF EXISTS leave_status_enum CASCADE;

-- Create new leave_status_enum
